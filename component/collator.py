...
class CompDataCollator(object):
    ...
    def __call__(self, batch: List[Dict[str, Any]]) -> Dict[str, Any]:
        # 找出batch中的最大长度
        lengths = [max(len(x[0]['input_ids']),len(x[1]['input_ids']))  for x in batch]
        # 取出batch中的最大长度，如果超过max_seq_length，则取max_seq_length
        batch_max_len = min(max(lengths), self.max_seq_length)
        # batch_max_len = self.max_seq_length
        ...
        # truncate and padding
        for pair in batch:
            input_ids_list, attention_mask_list, target_mask_list = [], [], []
            for x in pair:
                input_ids = x['input_ids']
                attention_mask = x['attention_mask']
                target_mask = x['target_mask']
            ...
            target_mask_batch.append(target_mask_list)
            # labels_batch.append(x['labels'])
